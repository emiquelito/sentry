...
class ExternalIssueForm extends AsyncComponent {
  ...
  getEndpoints() {
    ...
    return [['integrationDetails', integration.url]];
  }
...
export class SentryAppExternalIssueForm extends React.Component {
  ...
  render() {
    ...
    return (
      <Form
        apiEndpoint={`/sentry-app-installations/${sentryAppInstallation.id}/external-issues/`}
        apiMethod="POST"
      >
        <input type="hidden" name="action" value={this.props.action} />
        <input type="hidden" name="groupId" value={this.props.group.id} />
